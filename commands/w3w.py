...
def w3w(ctx, latitude, longitude):
    ...
    def command(latitude, longitude):
        import re
        if re.search('[a-zA-Z]', latitude) or re.search('[a-zA-Z]', longitude): 
            ctx.send(Response(embed={"title": "Error","description": "Invalid Latitude or Longitude"}))
            return
        ...
        if not regexlat or not regexlng: 
            ctx.send(Response(embed={"title": "Error","description": "Invalid Latitude or Longitude"}))
            return
