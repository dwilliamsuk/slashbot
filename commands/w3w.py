...
def w3w(ctx, latitude, longitude):
    ...
    if not regexlat or not regexlng: return Response(embed={"title": "Error","description": "Invalid Latitude or Longitude"})
    ...
    if resp.status_code == 400:
        jresp = resp.json()
        return Response(embed={"title": "Error","description": f"{jresp['error']['message'].capitalize()}","footer": {"text": f"Code: {jresp['error']['code']}"}})
    if resp.status_code != 200:
        return Response(embed={"title": "Error","description": "Try again later"})
    jresp = resp.json()
